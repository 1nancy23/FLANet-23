...
parser.add_argument('--dataset_root',
                    default='/root/autodl-tmp/VOCdevkit',
...
def train():
    ...
    if args.resume:
        ...
        weights_dict=torch.load(args.resume)
        # del_keys = ["conf.0.weight","conf.0.bias",
        #            "conf.1.weight","conf.1.bias",
        #            "conf.2.weight","conf.2.bias",
        #            "conf.3.weight","conf.3.bias",
        #            "conf.4.weight","conf.4.bias",
        #            "conf.5.weight","conf.5.bias",]
        # for k in del_keys:
        #     del weights_dict[k]      
        sme_net.load_state_dict(weights_dict, strict=False)

    # load backbone
    else:
        vgg_weights = torch.load(args.save_folder + args.basenet, map_location='cpu')
        print('Loading base network...')
        del_keys = ["0.weight"]
        for k in del_keys:
            del vgg_weights[k] 
        sme_net.vgg.load_state_dict(vgg_weights, strict=False)
        sme_net.vgg2.load_state_dict(vgg_weights, strict=False)
    ...
    if not args.resume:
        ...
        sme_net.extras_2.apply(weights_init)
        ...
        for j in range(2):
            ...
            sme_net.Erase2[j].apply(weights_init)
    ...
    for iteration in range(args.start_iter, cfg['max_iter']):
        ...
        if iteration % 1 == 0:
            ...
            # print('iter:{0} ||Loss:{1:0.4f}||  ||Conf_Loss:{2:0.4f}||  ||Loc_Loss:{3:0.4f}||  ||Off_loss:{4:0.4f}|| ||Lr{4:0.10f}||\n'
            #       .format(iteration, loss.item(), loss_c.item(), loss_l.item(), loss_f.item(),optimizer.param_groups[0]["lr"]))
            print('iter:{0} ||Loss:{1:0.4f}||  ||Conf_Loss:{2:0.4f}||  ||Loc_Loss:{3:0.4f}||  ||Lr{4:0.10f}||\n'
                  .format(iteration, loss.item(), loss_c.item(), loss_l.item(),optimizer.param_groups[0]["lr"]))
            per_time = 0
        ...
        if iteration%1000==0:
            torch.save(sme_net.state_dict(),
                       args.save_folder + '' + 'fuben' + '.pth')
        if cfg['max_iter'] - iteration <= 2:
            torch.save(sme_net.state_dict(),
                       args.save_folder + '' + 'supernewSME200' + str(iter_count) + '.pth')
            iter_count += 1
